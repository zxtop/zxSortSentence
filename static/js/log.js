        ...
        * */
        ...
        ['push', 'pop', 'shift', 'unshift', 'splice', 'sort', 'reverse'].forEach(function (method) {
            ...
            newArrProto[method] = function mutator() {
                ...
                if (errorArr.length > 0 && method === 'pop') {
                    ...
                    if (errorArr.length === 1) {
                        console.log(loadcomplete, device); //前台console.log输出
                        $.ajax({
                            type: "post",
                            url: urlData,
                            contentType: "application/json",
                            ...
                            success: function success(data) {
                                console.log("load-device日志输出:课件运行设备、加载完成状态");
                                sendStatus = true;
                                errorArr.pop();
                            },
        ...
        $('img').each(function (index, item) {
            ...
            if (/.*[\u4e00-\u9fa5]+.*/.test(newImg.src)) {
                console.log("课件中有中文名称的素材");
                newImg.onerror = function (e) {
                    errorArr.unshift(dataAssembling({
                        msg: '该图片url中有中文字符，请使用英文字符',
                        url: window.location.href,
